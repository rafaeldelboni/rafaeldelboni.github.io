...
(defn new-history
  ([]
   (new-history ""))
  ([path-prefix]
   (new-history path-prefix (-> (TokenTransformer.) set-retrieve-token! set-create-url!)))
  ([path-prefix transformer]
   (-> (Html5History. js/window transformer) (update-history! path-prefix))))
...
(defn pushy
  ...
  [dispatch-fn match-fn &
   {:keys [path-prefix processable-url? identity-fn prevent-default-when-no-match?]
    :or   {path-prefix                    ""
